...
void dm_motor_init(void)
{
    ...
    motor[Motor1].mst_id = 0x01;        // 实际没有用上，只做标识作用
    ...
    motor[Motor2].mst_id = 0x03;        // 实际没有用上，只做标识作用
...
void fdcan1_rx_callback(void)
{
	...
	switch (rec_id)
	{
 		case 0x01: dm_motor_fbdata(&motor[Motor1], rx_data); receive_motor_data(&motor[Motor1], rx_data); break;
		case 0x03: dm_motor_fbdata(&motor[Motor2], rx_data); receive_motor_data(&motor[Motor2], rx_data); break;
	}
}
